...
def build_script() -> Script:
    return (
        Script("GPS-style PRN (C/A Gold code) transmitter, file-replay at high "
               "sample rate. Transmit only into an authorised, shielded setup.")
        ...
        .number("-Code-rate", "--code_rate", unit="Mcps", min=0.1, max=20.0,
                ...
                help="Spreading-code chip rate. Live: swaps between the prebuilt "
                     "1.023 and 10.23 Mcps period files.")
        .number("-Gain", "--gain", unit="dB", min=0, max=89.75, default=50,
                required=True, live=True, help="USRP TX gain.")
        ...
        .number("-Center-frequency", "--freq", unit="Hz", min=70e6, max=6e9,
                ...
                help="RF carrier. Fixed per run (default L1).")
        .number("-Sample-rate", "--samp_rate", unit="MHz", min=1.0, max=61.44,
                default=40.0,
