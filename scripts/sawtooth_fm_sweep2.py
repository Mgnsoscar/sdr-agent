...
def build_buffer(samp_rate: float, cf: float,  bw: float, chirp_freq: float,
    ...
    X = np.fft.fft(x_shifted)

    freq = np.fft.fftfreq(len(x_shifted), 1 / samp_rate)

    # Bandpass example
    X[(freq < cf - bw/2) | (freq > cf + bw/2)] = 0

    chirp = np.fft.ifft(X).astype(np.complex64)
...
class ChirpTx(gr.top_block):
    def __init__(self, center_freq: float, bw: float, chirp_freq: float,
        ...
        self.usrp = uhd.usrp_sink(
            device_args,
            uhd.stream_args(cpu_format="fc32", channels=[0], num_send_frames=256),
        )
