...
`(window_ms, count)`; `ManagedProcess._underflow_fault_detail(text)` keeps a per-process streak. **The criterion is a
RATE (1.36.3):** GR's "last N ms" is the time since its PREVIOUS report (≥ 750 ms), not a fixed window — a saturated
stream reports every ~750 ms with thousands, a healthy one with a hiccup reports once per tens of seconds with a
handful; 1.36.2 summed those windows and stopped healthy tasks on ONE sparse report (`1 reports over 17.3 s (~0
underflows/s)`, owner: "18 underflows in 6 seconds is not worth stopping the task for"). Now a report at or above
**`UNDERFLOW_FAULT_RATE`** (200/s, `SDR_UNDERFLOW_FAULT_RATE`; observed ~3/s healthy vs ~10,000/s broken) is HEAVY
and its window counts; a lighter one is ignored (a light window ≥ the gap ends the streak); a heavy report within
`UNDERFLOW_GAP_S` 3 s / 1.5 polls of the previous continues the streak; the detail comes once the heavy windows cover
**`UNDERFLOW_FAULT_S`** (4 s = the 6th consecutive 750 ms report; `SDR_UNDERFLOW_FAULT_S`, 0 disables); `_scan_task_health` then goes through the
...
STALLED` stays reserved. `AGENT_VERSION 1.36.1 → 1.36.2 → 1.36.3`; no client change (it already renders `rf_fault` +
the detail). Tests: `tests/test_underflow_fault.py` (parser; the three screenshot reports never fault; light reports
don't count, a long light window resets, exactly-at-limit is heavy, the rate knob at 0 restores the window-sum reading;
streak/burst/accumulate/gap/knob-0/signature-wins; `start()` resets; LIVE over the real health loop: fault + stop, and
auto-restart → relaunch → trip). Suite 718 → 732. The 1.36.2 field run confirmed fault → auto-drop → relaunch on hardware.
