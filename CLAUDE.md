...
- **Capabilities + version:** a new client-visible feature adds a string to
  ...
  in `config.py` (bumped to `1.14.0` for opt-in measured-curve extrapolation).
...
(`AGENT_VERSION` → `1.14.0`, a safety gate: an older agent clamps → client range wouldn't match). The
